...
def make_resnet(num_blocks=3, num_filters=32, num_outputs=1, d1=64, d2=64, word_size=16, ks=3,depth=5, reg_param=0.0001, final_activation='sigmoid'):
...
def train_simon_distinguisher(num_epochs, num_rounds=7, depth=1,Diff=(0x200,0x880),BS=5000):  
    ...
    X, Y = sp.make_train_data_PDID(10**7,num_rounds,diff = Diff); 
    sum = 0;
    for i in range(10**7):
       sum+=Y[i]
    print(sum)
    X_eval, Y_eval = sp.make_train_data_PDID(10**6, num_rounds,diff = Diff); 
    ...
    X_test, Y_test = sp.make_train_data_PDID(10**6, num_rounds, Diff);  
